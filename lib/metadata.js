...
const pubkey = require('../bitcoin-consensus-encoding/lib/pubkey')
...
function encode (proof, schema, buf, offset) {
  ...
  if (proof.pubkey) pubkey.encode(proof.pubkey, buf, offset)
...
function encodingLength (proof, schema) {
  ...
  if (proof.pubkey) length += pubkey.encodingLength(proof.pubkey)
  return length
}
